    ...
    with gr.Blocks(title="Modular RAG System", theme=gr.themes.Soft()) as interface:
        ...
        **Intelligent Document Processing & Querying**
        ...
        def suggest_ontology_improvements():
            ...
            try:
                if not ontology_editor:
                    return None, [], "❌ Please load ontology first"
                ...
                return suggestions, choices, "✅ Suggestions generated"

            except Exception as e:
                return None, [], f"❌ Error generating suggestions: {str(e)}"

        def apply_selected_suggestion(suggestion_idx):
            """Apply a selected suggestion."""
            try:
                if not ontology_editor or not current_suggestions:
                    return "❌ Please generate suggestions first"

                if not suggestion_idx:
                    return "❌ Please select a suggestion"

                # Parse suggestion index
                try:
                    idx = int(suggestion_idx.split(':')[0]) - 1
                except:
                    return "❌ Invalid suggestion selection"
                ...
                import asyncio
                success, message = asyncio.run(ontology_editor.apply_suggestion(suggestion))
        ...
        suggest_improvements_btn.click(
            ...
            inputs=[],
            outputs=[improvement_suggestions, selected_suggestion, edit_result]
        )
