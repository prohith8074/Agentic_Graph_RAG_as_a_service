...
class OpikTracer:
    ...
    def track_llm_call(self, provider: str, model: str, prompt: str, response: str,
        ...
        try:
            with opik.track(name=f"{provider}_llm_call", metadata=metadata) as span:
    ...
    def track_query_routing(self, query: str, routing_decision: Dict[str, Any],
        ...
        if not self.is_enabled(): return
        try:
            with opik.track(name="query_routing", metadata=routing_decision) as span:
                span.log_input(query)
                span.log_output({"final_answer": final_answer})
        except Exception as e:
    ...
    def track_pipeline_step(self, step_name: str, input_data: Any,
        ...
        if not self.is_enabled(): return
        try:
            with opik.track(name=f"pipeline_{step_name}", metadata={"duration_seconds": duration}) as span:
                span.log_input(str(input_data)[:1000])
                span.log_output(str(output_data)[:1000])
        except Exception as e:
    ...
    def log_error(self, error_type: str, error_message: str, **metadata) -> None:
        ...
        if self.is_enabled():
            try:
                with opik.track(name=error_type, metadata=metadata) as span:
                    span.log_output(error_message)
            except Exception as e:
